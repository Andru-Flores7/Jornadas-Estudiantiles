# 🏆 Manual de Uso: Sistema de Puntuación Jornadas Estudiantiles 2026

Este documento explica el funcionamiento del sistema para asegurar una competencia justa y transparente.

---

## 🛡️ Rol del Administrador

El administrador es el encargado de la logística del sistema y tiene control total sobre la configuración:

1.  **Configuración del Encuentro**: Antes de empezar, el admin debe ingresar los nombres de los equipos y los nombres de los jurados. Al hacer clic en **"Guardar y Sincronizar"**, estos nombres aparecerán automáticamente en todas las pantallas.
2.  **Monitoreo en Vivo**: El tablero principal muestra en tiempo real los puntos acumulados por categoría y el estado de envío de cada jurado.
3.  **Descarga de Resultados**: Una vez finalizado el encuentro, el admin puede descargar un **Comprobante PDF** que sirve como acta oficial de los resultados.
4.  **Limpiar Evento**: Al terminar un encuentro, el botón de "Limpiar" borra todos los puntajes y resetea los nombres para el siguiente partido.

---

## ⚖️ Sistema de Puntuación (Lógica de Consenso)

Para evitar que los puntos se "tripliquen" al tener tres jurados, el sistema utiliza una **Lógica de Consenso por Mayoría** en las categorías objetivas:

### 1. Juegos (6 pts c/u)
*   Hay 3 juegos por encuentro.
*   Cada jurado marca quién ganó el juego en su planilla.
*   **Cómputo**: El sistema mira los votos de los 3 jurados. El equipo que tenga al menos 2 votos a favor gana los 6 puntos de ese juego.

### 2. Ganadores de Categoría (Popurrí, Mascota, Ritmos, Videoclip)
*   **Apreciación Individual**: Cada jurado evalúa y decide un ganador según su criterio.
*   **Cómputo por Mayoría**: El equipo que sea declarado ganador por la mayoría de los jurados (ej: 2 de 3) se lleva el premio único de la categoría. Esto evita que los puntos se dupliquen o tripliquen.

### 3. Puntos de Desempeño (Ritmos y Videoclip)
*   Las calificaciones numéricas (1-5 o 1-8) de **todos** los jurados se **suman**. Esto premia la calidad percibida por cada juez de forma acumulativa.

---

## 📝 Rol del Jurado

1.  **Ingreso**: Cada jurado debe entrar con su acceso asignado por el administrador.
2.  **Carga de Puntos**: A medida que transcurre el evento, el jurado completa su planilla. Los datos se guardan automáticamente.
3.  **Envío Final**: Una vez completada toda la evaluación, debe presionar **"🚀 ENVIAR RESULTADOS"**.

---

## 💡 Preguntas Frecuentes

*   **¿Por qué mis puntos no coinciden exactamente con la suma de mi planilla?**
    El total del Administrador aplica la lógica de consenso. Si tú votaste por el Equipo A en un juego, pero los otros dos jurados votaron por el Equipo B, los puntos irán al Equipo B (quien realmente ganó el juego).
*   **¿Qué pasa si solo hay un jurado cargando datos?**
    El sistema tomará la decisión de ese único jurado como la "mayoría" hasta que los demás envíen sus votos.
